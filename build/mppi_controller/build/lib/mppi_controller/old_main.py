...
@njit
def my_func(samples, cost_constr, iterations, human_data, p_final_cart):
    for i in range(iterations):
        response = casadi_f0(samples[i,0:6], samples[i,6:12], human_data, p_final_cart)
...
class MPPIController(Node):
    ...
    def set_human_data(human_data_msg):
        global human_data
        human_data = human_data_msg

    def human_data_callback(self, msg):
        temp = msg.data
        temp = np.array(temp)
        temp = temp.reshape((14, 4))
        human_data_msg = torch.tensor(temp, device='cuda')
        self.set_human_data(human_data_msg)
    ...
    def running_cost(self, state, action=None):
        ...
        p_final_cart = np.array([0.0, 0.0, 0.0])
        human_data_np = human_data.cpu().numpy()
        cost_constr = my_func(state_array, cost_constr, iterations, human_data_np, p_final_cart)
