...
from mppi_controller.mppi_utils import generate_gaussian_halton_samples, scale_ctrl, cost_to_go
...
class MPPIPlanner(ABC):
    ...
    def __init__(self, cfg: MPPIConfig, nx: int, dynamics: Callable, running_cost: Callable, prior: Optional[Callable] = None):
        ...
        self.noise_sigma = torch.tensor(cfg.noise_sigma, device=cfg.device, dtype=torch.float)
        ...
        self.u_max = torch.tensor(cfg.u_max, device=cfg.device, dtype=torch.float)
        self.u_min = torch.tensor(cfg.u_min, device=cfg.device, dtype=torch.float)
    ...
    def command(self, state, updated_sigma=None):
        """
            Given a state, returns the best action sequence
        """
        if updated_sigma:
            # Convert lists in cfg to tensors and put them on device
            self.noise_sigma = torch.tensor(updated_sigma, device=self.tensor_args['device'], dtype=self.tensor_args['dtype'])
            self.noise_sigma_inv = torch.inverse(self.noise_sigma)
            self.noise_dist = MultivariateNormal(
                self.noise_mu, covariance_matrix=self.noise_sigma
            )
    ...
    def _compute_rollout_costs(self, perturbed_actions):
        ...
        n_s, m_s, k_s = states.shape
        states = states.reshape(n_s * m_s, k_s)
        
        cost_batch = self._running_cost(states)
        
        cost_batch = cost_batch.reshape(n_s, m_s)
        states = states.reshape(n_s, m_s, k_s)
        
        cost_batch = cost_batch.type(torch.cuda.FloatTensor)
               
        cost_horizon = cost_batch
        cost_samples = torch.sum(cost_batch, dim=1)
        
        #print('Shapes:', states.shape, cost_horizon.shape, cost_samples.shape)
