...
private:
    ...
    void timer_callback()
    {
            ...
            //std::cout << "Start_solver" << std::endl;
            int number_of_current_steps = 5;
            ...
            RCLCPP_INFO(this->get_logger(), "calc time [%.2f], j_error [%.4f], dt [%.4f], cost [%.4f]", results[13], j_error, results[0], results[2]);
            now = this->get_clock()->now();
            ...
            joint_vel_values.data.push_back(number_of_current_steps); // fixed step length [N_steps, q_0[7], dt, q_1[7], dt... q_N[7], dt]
            for (int i = 0; i < 10*number_of_current_steps; i++) joint_vel_values.data.push_back(full_trajectory[i]);
