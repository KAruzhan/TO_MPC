...
public:
    RobotCommandPublisher() : Node("robot_command_publisher"), count_(0)
    {
        ...
        MyFile.open("solver_to_stats1.csv"); //, std::ofstream::out | std::ofstream::app);

        nmpc_solver = std::make_shared<my_NMPC_solver>(1, number_of_current_steps);
...
private:
    ...
    void timer_callback()
    {
            ...
            // check distance to goal (angle-wrapped)
            auto angle_err = [](double g, double c) {
                double e = fmod(g - c, 2*M_PI);
                if (e >  M_PI) e -= 2*M_PI;
                if (e < -M_PI) e += 2*M_PI;
                return fabs(e);
            };
            double j_error = angle_err(goal_position[0], current_position[0]);

            for (int i=1;i<6;i++) {
                double ei = angle_err(goal_position[i], current_position[i]);
                if (ei > j_error) j_error = ei;
            }
            //
            ...
            RCLCPP_INFO(this->get_logger(), "pose: %.5f, %.5f, %.5f, %.5f, %.5f, %.5f", current_position[0], current_position[1],
             ...
             nmpc_solver->solve_my_mpc(current_position, 
                current_velocity,
                // human_sphere, 
                goal_position, tracking_goal, cgoal, results, solver_weights);
            ...
            // Use stage-1 predicted velocity (x1.qd) as the command, not stage-0 state velocity.
            ...
            MyFile << now.seconds() << "," << now.nanoseconds() << "," << results[14] << "," << results[15] << ","
            //  << smallest_dist << "," << max_linear_vell << "," 
    ...
    {   
        ...
        constexpr double OFFSET = 0.0;     // No intentional tracking delay

        // n_steps goal_1(6) time_1 goal_2(6) ... goal_n(6)
        if (solution_msg.data.size() < static_cast<size_t>(sample_line + 1)) {
            return;
        }
        ...
        else 
        {
            ...
            // Remap trajectory to 0.05 s intervals over a 0.5 s window.
            for (int i=0; i<MAX_STEPS; i++) 
            {
                double current_mark = i * TIME_STEP + OFFSET;
                ...
                for (int j=1; j<steps+1; ++j) 
                {
                    ...
                    {
                        int t1 = j-1; int t2 = j;
                        double denom = time_vector[t2] - time_vector[t1];
                        double tau = denom > 1e-9 ? (current_mark-time_vector[t1]) / denom : 0.0;
      ...
      for (int i=0;i<6;i++) {
          for (int j=0;j<6;j++) {
              if (msg->name[i]==joint_names[j]) {
                double p = fmod(msg->position[i], 2*M_PI);
                if (p >  M_PI) p -= 2*M_PI;
                if (p < -M_PI) p += 2*M_PI;
                this->current_position[j] = p;
